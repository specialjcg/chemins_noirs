...
This workspace hosts three Rust crates (`backend`, `frontend`, `shared`) that ship together. Keep changes scoped and well-documented so other agents can move quickly.

## Project Structure & Module Organization
`backend/` is the binary crate (`src/main.rs`) where services or CLIs live. `frontend/` and `shared/` are library crates (`src/lib.rs`) intended for reusable UI/state helpers and cross-cutting logic. Shared assets or fixtures should live alongside the crate they exercise; avoid creating ad-hoc top-level folders so `cargo` metadata stays authoritative.
...
- `trunk serve --open` (run from `frontend/`) – compiles the Seed/WebAssembly frontend and serves it with hot reload. Requires `wasm32-unknown-unknown` target (`rustup target add wasm32-unknown-unknown`) and the `trunk` CLI.
...
Rust 2024 edition defaults apply: 4-space indentation, trailing commas where rustfmt inserts them, and one `use` group per module. Keep modules and files in `snake_case`, public types in `PascalCase`, constants in `SCREAMING_SNAKE_CASE`, and unit tests named `mod tests` at the end of each file. When adding crates or binaries, register them in the root `Cargo.toml` workspace to keep dependency resolution consistent.

## Testing Guidelines
Favor small unit tests colocated with the code plus integration tests in `tests/` for cross-crate behavior. Name tests after expected behavior (`fn rejects_empty_payload()`), and set up fixtures through helper functions in `shared` when multiple crates reuse them. Run `cargo test -- --nocapture` if you need stdout during debugging. Aim to cover new branches and error paths; note uncovered follow-ups in the PR if exhaustive testing would be disproportionate.

## Commit & Pull Request Guidelines
Commit subjects should be imperative (`Add upload endpoint`), under 72 characters, and mention the touched crate when helpful (`backend: add upload endpoint`). Reference issues with `#ID` in either the body or footer. Every PR should describe the change, risks, and manual test notes, plus link related issues or screenshots for UI work. Before opening a PR, run `cargo fmt`, `cargo clippy --all-targets --all-features`, and `cargo test`; paste the relevant command outputs so reviewers can trust the state.
# Repository Guidelines
...
## Testing Guidelines
Use `cargo test` locally for fast iterations; target specific scenarios with `cargo test --test lib_integration_test`. Integration suites rely on the pretrained weights shipped in `model_weights/`, so avoid mutating those files in-place—stage replacements under the same names. Record new regression cases by mirroring the naming pattern in `tests/ui_reactivity_regression_test.rs`. Capture coverage reports by running `run_all_tests.sh` and reviewing artifacts under `coverage/`.
...
#### Data Flow (text)
- Player/Web clients → `servers::WebUiServer` (static assets + WebSocket) → `services::session_service` for lobby/session lifecycle.
- gRPC calls (`takeiteasygame.v1`) → `services::game_service::mcts_integration` → `services::game_manager` → pure `game::*` logic.
- AI decision path: `game_manager` → `mcts::algorithm` → `neural::manager` (policy/value nets) ↔ `strategy::*` heuristics → updated session state.
- Training mode: `training::session` streams self-play data → `neural::training` optimisers → persists into `game_data_*.pt` and `model_weights/`.
...
- Observability limited to flexi_logger output—add structured tracing and metrics before public hosting.
...
- CI/automation story unclear—no visible workflow enforcing `cargo fmt`, `clippy`, or dataset integrity checks.
...
#### Strengthening Plan (RED → GREEN → REFACTOR)
...
- Performance regression tracking is absent—self-play simulations run without baseline measurements beyond the `compare_mcts` binary.
...
- Multiplayer flow: CLI → `servers::WebUiServer` (static host) & `servers::GrpcServer` → `services::game_service` → `services::game_manager`.
- Training flow: CLI → `training::session` → `neural::training` optimisers; artifacts stored under `game_data_*` & `model_weights/`.
...
- Replace panic-prone tensor loading with typed error handling
    - ⭐ Introduce a `ModelAssets` helper in `src/neural/manager.rs` returning `Result<ModelAssets, ModelLoadError>`
        - ⭐ Extend `TakeItEasyError` with a model-loading variant carrying the source `tch::TchError`
        - ⭐ Move path fallback logic from `src/main.rs` into the helper with configurable search order
...
- Add integration coverage for missing-weight scenarios
    - ⭐ Create temporary fixture directories via `tempfile`
    - ⭐ Assert multiplayer startup fails gracefully with descriptive errors
...
- Building without libtorch available fails at link time—document `LD_LIBRARY_PATH` setup or add feature-gated stubs for CI.
